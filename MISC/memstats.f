...
c-- read stat file
      ...
      if(.not.exists .and. .not.skipwarn) then
       call warn('memstats','file missing: /proc/self/stat,no mem info')
       skipwarn = .true.
       return
      endif
c-- read
...
c
      write(0,'(a,i5,2(i6,"MB"))')
     &  ' Memory usg (rank,vsize,rss):',impi,
     &  stat%vsize/1024**2,stat%rss*pagesize/1024**2
c
      return
c
 6    call warn('memstats','read error: eof: /proc/self/stat')
      close(4)
      skipwarn = .true.
c
