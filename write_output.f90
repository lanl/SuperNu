...
!==============
  ...
  do k=1,flx_nom
  do j=1,flx_nmu
     arr = merge(flx_luminos(1,:,j,k),0d0,flx_luminos(1,:,j,k)>1d-99)
     write(4,'(1p,10000e12.4)') arr !prevent fortran number truncation, e.g. 1.1234-123
  enddo
  enddo
  close(4)
  ...
  do k=1,flx_nom
  do j=1,flx_nmu
     arr = flx_luminos(3,:,j,k)
     write(4,'(10000i12)') nint(arr)
  enddo
  enddo
  close(4)
  ...
  do k=1,flx_nom
  do j=1,flx_nmu
     arr = flx_luminos(2,:,j,k)
     write(4,'(1p,10000e12.4)') arr
  enddo
  enddo
  close(4)
...
!-- gamma flux
  ...
  do k=1,flx_nom
  do j=1,flx_nmu
     write(4,'(1p,10000e12.4)') merge(flx_gamluminos(1,j,k),0d0,flx_gamluminos(1,j,k)>1d-99) !prevent fortran number truncation, e.g. 1.1234-123
  enddo
  enddo
  close(4)
