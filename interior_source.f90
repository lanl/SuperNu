...
!##################################################
  ...
  do ipart = prt_nsurf+1, prt_nsurf+prt_nexsrc
     ...
     do while (.not.isnotvacnt)
        if (irused(ir)<gas_nvolex(ir,1,1)) then
           ...
           prt_particles(ivac)%rsrc = (r3*gas_xarr(ir+1)**3 + &
           ...
           if (((gas_sig(ir,1,1)+gas_cap(iig,ir,1,1))*gas_dxarr(ir)* &
                help < prt_tauddmc) &
                .or.(in_puretran)) then
              if(gas_isvelocity) then
...
!-- Thermal volume particle instantiation: loop
  ...
  do ipart = prt_nsurf+prt_nexsrc+1, prt_nnew
     ...
     do while (.not.isnotvacnt)
        if (irused(ir)>=gas_nvol(ir,1,1)) then
!-- do what?
           ir = ir + 1
        else
!-- or what?
...
!            wl0 = pc_h*pc_c/(xx0*pc_kb*gas_temp(ir,1,1))
           ...
           do while (r2 > r1)
              ...
              r3 = (r0-gas_xarr(ir))/gas_dxarr(ir)
           ...
           if (((gas_cap(iig,ir,1,1)+gas_sig(ir,1,1))*gas_dxarr(ir)* &
                help < prt_tauddmc) &
                .or.(in_puretran)) then
              if(gas_isvelocity) then
                 prt_particles(ivac)%esrc = Ep0*(1.0+r0*mu0/pc_c)
