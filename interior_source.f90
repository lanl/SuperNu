...
!##################################################
  ...
  real*8 :: r1, r2, r3, uul, uur, uumax, mu0, x0, y0, z0, ep0, wl0
...
!-- check for available particle space to populate in cell
     do i=i,gas_nx
       if (ijkused(i,j,k)<gas_nvol(i,j,k)) exit
     enddo
!--
     ijkused(i,j,k) = ijkused(i,j,k)+1
     !Calculating Group
     ...
     do while (r2 > r1)
        r3 = rand()
        prt_tlyrand = prt_tlyrand+1
        x0 = (r3*gas_xarr(i+1)**3+(1.0-r3)*gas_xarr(i)**3)**(1.0/3.0)
     ...
     if (((gas_cap(iig,i,j,k)+gas_sig(i,j,k))*dx(i)* &
          help < prt_tauddmc) &
          .or.(in_puretran)) then
        if(gas_isvelocity) then
           ptcl%esrc = ep0*(1.0+x0*mu0/pc_c)
           ptcl%ebirth = ep0*(1.0+x0*mu0/pc_c)
!-- velocity effects accounting
           gas_evelo=gas_evelo-ep0*x0*mu0/pc_c
!
        !(rev 120)
           ptcl%wlsrc = wl0/(1.0+x0*mu0/pc_c)
        !
           ptcl%musrc = (mu0+x0/pc_c)/(1.0+x0*mu0/pc_c)
        else
           ptcl%esrc = ep0
           ptcl%ebirth = ep0
        !
