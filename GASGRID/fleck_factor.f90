...
!-- calculating modified Fleck factor
  do i=1,gas_ncell
     ...
     if(gas_temp(i)<=0d0.or.gas_bcoef(i)==0d0) then
        beta = 0d0
     elseif(gas_siggrey(i)<=0d0.or.siggreyalt(i)<=0d0) then
...
!-- convert from per gram
           dlogsig = log(gas_siggrey(i)/(siggreyalt(i)*gas_rho(i)))/&
     ...
     gas_fcoef(i) = 1.0/(1.0+tsp_alpha*beta*pc_c*tsp_dt*gas_siggrey(i))
  enddo !i
