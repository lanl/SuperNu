...
SUBROUTINE diffusion1alt(z,g,r,mu,t,E,E0,hyparam,vacnt)
  ...
  r1 = RAND()
  tauA = ABS(LOG(r1)/(lspeed*gas_fcoef(z)*gas_sigmapg(g,z)))
  r1 = RAND()
  tauS = ABS(LOG(r1)/(lspeed*(1.0-gas_emitprobg(g,z))*(1.0-gas_fcoef(z))*gas_sigmapg(g,z)))
  r1 = RAND()
  tauR = ABS(LOG(r1)/(lspeed*gas_sigmar(g,z)))
  r1 = RAND()
  tauL = ABS(LOG(r1)/(lspeed*gas_sigmal(g,z)))
  ...
  ELSEIF (ddmct == tauS) THEN
     denom2 = gas_sigmap(z)-Ppick(g)*gas_sigmapg(g,z)
     DO ig = 1, gas_ng
