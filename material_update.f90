SUBROUTINE material_update
  ...
  gas_emat = 0.0
  DO ir = 1, gas_nr
     dtemp = gas_edep(ir)*3.0/(4.0*pi*gas_dr3arr(ir)*(gas_velno*1.0+gas_velyes*tsp_texp**3))
     dtemp = (dtemp-tsp_dt*gas_fcoef(ir)*gas_sigmap(ir)*lspeed*gas_ur(ir))/gas_bcoef(ir)
     !WRITE(*,*) dtemp
     gas_temp(ir) = gas_temp(ir)+dtemp
     !gas_ur(ir)=dtemp/(tsp_dt*lspeed*gas_sigmap(ir))
     ...
     gas_emat = gas_emat + Um*4.0*pi*gas_dr3arr(ir)*(gas_velno*1.0+gas_velyes*tsp_texp**3)/3.0
     ...
     !gas_edep(ir) = gas_edep(ir)*3.0/(4.0*pi*gas_dr3arr(ir)*(gas_velno*1.0+gas_velyes*tsp_texp**3))
  ENDDO
