        ...
        IF (puretran.EQV..FALSE.) THEN
           IF (gas_sigmapg(gsrc,zsrc)*gas_drarr(zsrc)*(gas_velno*1.0+gas_velyes*tsp_texp)<5.0_rknd) THEN
              IF (rtsrc == 2) THEN
                 ...
                 r1 = RAND()
                 musrc = 1.0-2.0*r1
                 musrc = (musrc+gas_velyes*rsrc/lspeed)/(1.0+gas_velyes*rsrc*musrc/lspeed)
                 Esrc = Esrc/(1.0-gas_velyes*musrc*rsrc/lspeed)
                 Ebirth = Ebirth/(1.0-gas_velyes*musrc*rsrc/lspeed)
              ENDIF
              rtsrc = 1
           ELSE
              rtsrc = 2
